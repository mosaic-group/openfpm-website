...
```sh
cd openfpm_data
mpirun -np 1 ../build/openfpm_data/src/mem_map --log_level=test_suite
cd ..

cd openfpm_devices
mpirun -np 1 ../build/openfpm_devices/src/mem --log_level=test_suite
cd ..

cd openfpm_io
mpirun -np 1 ../build/openfpm_io/src/io --log_level=test_suite
cd ..

cd openfpm_numerics
mpirun -np 3 ../build/openfpm_numerics/src/numerics --log_level=test_suite
cd ..

cd openfpm_vcluster
mpirun -np 3 ../build/openfpm_vcluster/src/vcluster_test --log_level=test_suite
cd ..

cd openfpm_pdata
mpirun -np 3 ../build/openfpm_pdata/src/pdata --log_level=test_suite
cd ..
```
